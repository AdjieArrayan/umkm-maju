...
<form action="{{ route('stock-outs.update', $stockOuts) }}" method="POST" class="space-y-5">
    @csrf
    @method('PUT')
    ...
    <div>
        ...
        <input type="date"
               name="date"
               value="{{ old('date', \Carbon\Carbon::parse($stockOuts->date)->format('Y-m-d')) }}"
        ...
        class="w-full h-11 rounded-lg border px-4 text-sm
            ...
            @foreach($items as $item)
                <option value="{{ $item->id }}"
                    {{ old('item_id', $stockOuts->item_id) == $item->id ? 'selected' : '' }}>
                    {{ $item->name }}
                </option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <textarea name="description"
                  rows="3"
                  class="w-full rounded-lg border px-4 py-2 text-sm dark:text-white/90">{{ old('description', $stockOuts->description) }}</textarea>
